...
```typescript
type PendingOrder = { createdAt: Date };
type PaidOrder = { paidAt: Date; amount: number };
type CancelledOrder = { cancelledAt: Date; reason: string };
type RefundedOrder = { refundedAt: Date; paidAmount: number; refundAmount: number };
