...
>TL;DR :  
>Traditionnellement, la modélisation de données se concentre sur les entités et leurs attributs.
Les modèles de données traditionnels finissent souvent en tables obèses, difficiles à maintenir.
>
>Le Graph-Driven Design (GDD) propose de centrer la modélisation non plus sur les entités, mais sur les relations, qui incarnent les interactions et l’évolution métier.
>
>Résultat : des données plus stables, une architecture plus souple, et un langage métier plus proche de la réalité.
{: .prompt-tip }
