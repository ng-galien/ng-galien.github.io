...
last_modified_at: 2025-11-02 14:30:00 +0100
...
Jusqu’ici, nous avons traité les relations comme des faits ou des états.
Mais dans un système vivant, une relation n’est pas seulement une donnée : c’est aussi une invitation à agir.
C’est ici qu’intervient la logique hypermédia.

### Des données aux capacités

Dans une API hypermédia, chaque relation devient un lien porteur de capacité.
Elle ne se contente pas de décrire le système ; elle propose au client les actions possibles à partir d’un état donné.

Exemple :
Une commande liée à une relation APPROVED_BY pourra exposer un lien cancel, ship, ou invoice.
Le client n’a plus besoin de connaître les règles métiers pour construire les URLs : il les découvre dans le graphe.

Ainsi, le graphe ne transporte plus seulement la structure des données, mais l’état dynamique du domaine et ses transitions.

### Lien entre température et hypermédia

On peut relire la chaleur des données à la lumière de l’hypermédia :

| Type de donnée | Température | Représentation | Comportement |
|----------------|-------------|----------------|--------------|
| Donnée froide  | Stable      | Nœud ou attribut | Représente l’identité, ne change pas via lien |
| Donnée tiède   | Modérément stable | Nœud ou relation versionnée | Modifiable par des liens de mise à jour |
| Donnée chaude  | Volatile, contextuelle | Relation active | Porte des actions (approve, assign, cancel) |

Les liens hypermédia deviennent donc la surface de pilotage des données chaudes : ce sont eux qui expriment la dynamique du métier.

> La frontière entre donnée et action s’efface : une relation “vivante” peut être à la fois un fait constaté et une possibilité d’évolution.
{: .prompt-info }

### L’action comme continuité du graphe

Dans ce paradigme, une action métier n’est plus un endpoint REST isolé (POST /approve)
mais une extension du graphe : un lien qui, lorsqu’il est activé, ajoute une nouvelle relation.
...
sequenceDiagram
  ...
  API->>Graph: crée (Order)-[:APPROVED_BY]->(User)
  Graph-->>API: nouvel état dérivé
  API-->>Client: représentation + liens mis à jour
```

Le graphe devient ainsi le cœur unique du système :
il décrit le présent, garde la mémoire du passé et expose les transitions possibles vers le futur.

### Boucle de vie complète

1. Les nœuds définissent les entités stables.
2. Les relations décrivent les faits et les contextes.
3. Les liens hypermédia expriment les capacités d’évolution.

Ce triptyque permet de concevoir une architecture où les données, les règles et les actions partagent la même grammaire.
Le système devient explorable, auto-documenté et naturellement cohérent.

L’hypermédia n’est pas un vernis REST, c’est la traduction opérationnelle d’un graphe vivant :
un modèle où les données froides décrivent le monde,
les relations chaudes le font évoluer,
et les liens hypermédia lui donnent un futur.
{: .prompt-tip }
...
```mermaid
flowchart LR
  subgraph Sales [Sales (BC)]
    O((Order)) -->|HAS_LINE| L((Line))
    O -->|BILLED_BY| I((Invoice))
  end
  subgraph Catalog [Catalog (BC)]
    P((Product)) -->|LISTED_IN| C((Catalog))
    P -->|HAS_PRICE| R((Price))
  end
  subgraph Warehouse [Warehouse (BC)]
    P2((Product)) -->|STORED_IN| W((Warehouse))
  end
  Sales -. upstream/downstream .- Catalog
  Sales -. ACL .- Warehouse
```
...
sequenceDiagram
  ...
  App->>Graph: Tx: save(Order); add REL [:ORDER_PLACED]
  Graph-->>App: commit
  App->>Outbox: enqueue(OrderPlaced)
  Outbox-->>App: ack
```
...
### Anti‑Corruption Layer (migration legacy)

- **Désenchevêtrer** une table fourre‑tout : extraire le **nœud identité** + projeter les **faits** en **relations**.
- **Conserver** la table comme **source** le temps d’une transition ; créer des **vues** qui exposent le graphe.
- **Stratégie progressive** : coexistence des deux modèles + tests de non‑régression.

### Tests orientés propriétés

- Tests **d’invariants** : une relation invalide ne passe pas la transaction.
- Tests **de trajectoire** : une séquence d’actions conduit à un graphe attendu.
- **Property‑based testing** sur les transformations `[relationsₙ] → [relationsₙ₊₁]`.

-### Guide pratique (check‑list)

- Énonce l’**UL** (noms/verbes) → mappe **nœuds/relations**.
- Clarifie les **bounded contexts** → **sous‑graphes** + **context map**.
- Définis les **invariants** → contraintes sur relations + transactions.
- Expose les **capacités** via **liens hypermédia** (actions applicables).
- Publie les **domain events** via **Outbox** (idempotence, DLQ).
- Prépare les **read models** (projections) pour les usages de lecture.
